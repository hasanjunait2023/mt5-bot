...
def get_ea_performance():
    ...
    backtest/ea_performance_report.py which writes mt5_bridge/_ea_performance.json.
    """
    if not EA_PERF_PATH.exists():
        return {
            "available": False,
            "message": "No performance report yet. Run: python "
                       "backtest/ea_performance_report.py (writes _ea_performance.json).",
            "eas": [], "summary": {}, "account": {},
        }
    try:
...
def get_ict_results():
    ...
    Generated by: python backtest/backtest_runner.py --ict-wf
    ...
    if not ICT_WF_PATH.exists():
        return {
            ...
            "message": "No ICT walk-forward results yet. "
                       "Run: python backtest/backtest_runner.py --ict-wf",
            "symbols": {}, "promoted": [],
        }
    try:
