"""Dashboard API — TV Desk agents (Intraday Analyst + Session Scalper).
...
def intraday_state():
    state = _read_json(INTRADAY_DIR / "_state.json",
                       {"running": False, "mode": "intraday", "updated_at": None})
    events = _read_events(INTRADAY_DIR / "_events.jsonl", 60)
...
def intraday_events(limit: int = 60):
    return {"events": _read_events(INTRADAY_DIR / "_events.jsonl", max(1, min(limit, 500)))}
...
def intraday_perf():
    return _read_json(INTRADAY_DIR / "_perf.json", {"totals": {}, "by_symbol": {}, "by_session": {}})


# ── Session Scalper ──────────────────────────────────────────────────────────
@router.get("/session-scalp/state")
def scalp_state():
    state = _read_json(SCALP_DIR / "_state.json",
                       {"running": False, "mode": "scalp", "updated_at": None})
    events = _read_events(SCALP_DIR / "_events.jsonl", 80)
    return {"state": state, "latest": _latest_per_symbol(events), "count": len(events)}


@router.get("/session-scalp/events")
def scalp_events(limit: int = 80):
    return {"events": _read_events(SCALP_DIR / "_events.jsonl", max(1, min(limit, 500)))}
...
def scalp_perf():
    return _read_json(SCALP_DIR / "_perf.json", {"totals": {}, "by_symbol": {}, "by_session": {}})
