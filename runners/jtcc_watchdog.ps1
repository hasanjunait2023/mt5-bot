...
# Modes:
#   .\runners\jtcc_watchdog.ps1          single check
#   .\runners\jtcc_watchdog.ps1 -Loop    persistent loop (checks every IntervalSec)
...
$ProjectDir = if ($scriptDir -like "*\runners") { Split-Path -Parent $scriptDir } else { $scriptDir }
...
function Invoke-WatchdogCheck {
    ...
    if (-not $bridgeAlive) {
        ...
        & "$ProjectDir\runners\start_mt5_bridge.ps1" -Port 8090 -Force | Out-Null
    ...
    if ($anyDead) {
        ...
        & "$ProjectDir\runners\start_jtcc_agents.ps1" | Out-Null
