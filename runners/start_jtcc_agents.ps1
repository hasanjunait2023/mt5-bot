...
if ($wdRunning -and -not $Force) {
    ...
    $wdProc = Start-Process powershell -ArgumentList @("-NoProfile","-WindowStyle","Hidden","-ExecutionPolicy","Bypass","-File","$scriptDir\jtcc_watchdog.ps1","-Loop") -WindowStyle Hidden -PassThru
...
Write-Host "Stop all:  .\runners\stop_jtcc_agents.ps1" -ForegroundColor DarkGray
